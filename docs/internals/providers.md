...
A provider is the agent runtime that does the actual work. Codi supports several, and the
...
Codi does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before Codi's next refresh sees them.
