...
This guide is for people who want to use more than one Claude setup in Codi. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Codi](./install.md).
...
In Codi Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Codi uses Claude Code's normal config directory.

When you set this field, Codi points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Codi
...
Codi looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Codi looks.

Then add another Claude provider in Codi:
...
Codi only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Codi keeps separate config directories isolated
...
Add or edit a Claude provider in Codi Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Codi stores the value as a server secret and does not
...
Codi does not need a special Claude Code Router provider. Treat the router as a Claude
