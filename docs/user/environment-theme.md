...
machine running your Codi server does that, its themes appear in the theme library alongside
your own, and Codi follows them: change the desktop theme and Codi retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. Codi Mobile keeps its own
...
the standard Codi look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Codi state
...
exported from Codi (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Codi generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Codi theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Codi does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Codi never reads
