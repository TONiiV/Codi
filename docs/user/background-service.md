# Running Codi in the Background

On Linux and macOS, Codi can run as a background service for your user, so it is ready without
keeping a terminal open.

## Manage the Service

Install it with the latest Codi release:
...
Updating restarts Codi briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact Codi versions are installed separately, so a
...
want Codi to start in the background.
