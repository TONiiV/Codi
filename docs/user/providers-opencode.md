...
Codi uses the OpenCode setup on the connected environment. With a remote environment, its
OpenCode login and configuration apply, not the setup on your desktop or phone.

Codi requires OpenCode 1.14.19 or newer. It checks the server version before it loads models or
...
Without a server URL, Codi starts a local OpenCode server. The process inherits
`OPENCODE_SERVER_PASSWORD` from the environment. A password in the provider settings overrides
that environment value for both the local process and Codi.

With a server URL, Codi connects to that external server and uses only the password in the
...
Codi loads the model list when an enabled OpenCode provider starts and keeps the list in its
...
After changing an OpenCode login or configuration outside Codi, open **Settings > Providers**,
select the environment, and choose **Refresh provider status**. Changing the provider's
configuration in Codi also replaces that provider connection.
...
Codi does not own an external OpenCode server. Native configuration changes on that server can
require its own reload or restart before a refresh returns the new list.

If a refresh fails, Codi keeps the last known models, slash commands, and skills. Fix the
...
until the model metadata returns. Codi does not switch the thread to the first model in the
list.
