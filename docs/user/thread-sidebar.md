...
If reordering is unavailable for one environment, update the Codi server running in that
...
pill** fallback because their colors are not controlled by Codi.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While Codi is generating it, the action reads **Regenerating…** and cannot
